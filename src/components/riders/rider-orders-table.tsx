...
const FilterBar = React.memo(function FilterBar({
  ...
  return (
    <>
      <div className={cn("grid grid-cols-1 gap-4", hideStatusFilter ? "md:grid-cols-3" : "md:grid-cols-4")}>
        {/* Search */}
        <div className="relative flex flex-col gap-1.5">
          ...
          <div
            className={cn(
              "flex items-center border rounded-xs transition-colors !h-10 bg-white",
              appliedSearch ? "border-orange-400" : "border-gray-200"
            )}
          >
        ...
        {!hideStatusFilter && (
          <div className="relative flex flex-col gap-1.5">
            ...
            <Select value={draftStatus} onValueChange={(value) => setDraftStatus(value || "")}>
              <SelectTrigger
                className={cn(
                  "w-full !h-10 rounded-xs px-3 text-xs text-gray-500 bg-white shadow-none focus:ring-0 transition-colors",
                  appliedStatus ? "border-orange-400" : "border-gray-200"
                )}
              >
        ...
        <div className="flex relative flex-col gap-1.5">
          ...
          <Select value={draftDeliveryLocation} onValueChange={(value) => setDraftDeliveryLocation(value || "")}>
            <SelectTrigger
              className={cn(
                "w-full !h-10 rounded-xs px-3 text-xs text-gray-500 bg-white shadow-none focus:ring-0 transition-colors",
                appliedDeliveryLocation ? "border-orange-400" : "border-gray-200"
              )}
            >
        ...
        <div className="relative flex flex-col gap-1.5">
          ...
          <Popover>
            <PopoverTrigger
              id="date-picker-range-rider"
              className={cn(
                buttonVariants({
                  variant: "outline",
                  className:
                    "justify-start px-2.5 font-normal !h-10 w-full rounded-xs text-xs text-gray-500 transition-colors hover:bg-white hover:text-gray-500",
                }),
                appliedDateRange?.from ? "border-orange-400" : "border-gray-200"
              )}
            >
...
function TrackingCodeCell({ value }: { value: string }) {
  ...
  return (
    <div className="group w-full h-full flex items-center justify-center p-3">
      ...
      <span className="text-[#d85860] text-xs font-medium whitespace-nowrap">{value}</span>
    </div>
  );
}
        ...
        <DialogHeader>
          ...
          {trackingNumber && (
            <p className="text-[10px] text-gray-400 mt-0.5">
              Tracking: <span className="text-[#d85860] font-medium">{trackingNumber}</span>
            </p>
          )}
        </DialogHeader>
...
// ─── Dedicated Rider Orders Table Component ─────────────────────────────────

export function RiderOrdersTable({
  riderId,
  fixedStatus,
  ...
  const handlePrint = () => {
    ...
    const tableRows = ordersToPrint
      ...
      <tr>
        ...
        <td>${order.recipient_address}, ${order.recipient_city}${
          order.recipient_district ? ", " + order.recipient_district : ""
        }</td>
        <td>${order.payment_type}</td>
        <td>Rs. ${order.cod_amount}</td>
        <td>${order.status}</td>
        <td>${order.latest_status_comment ?? ""}</td>
      </tr>
    `
      )
      .join("");
...
<body>
  ...
  <h1>Rider Order Package Report</h1>
  <br/>
  ...
  const {
    ...
    error: pkgError,
    refetch: refetchPkg,
  } = useRiderPackageStats(startDate, endDate, showStats, riderId);
  ...
  const {
    ...
    error: commError,
    refetch: refetchComm,
  } = useRiderCommissionStats(startDate, endDate, showStats, riderId);
  ...
  const columns: ColumnDef<Order>[] = useMemo(
    () => [
      ...
      },
      {
        ...
        header: "S.N.",
        cell: ({ row }) => (
          <span className="text-gray-600 text-xs font-medium text-center block">{row.index + 1}</span>
        ),
        size: 50,
      },
      {
        ...
        cell: ({ row }) => {
                ...
                <Select
                  ...
                  onValueChange={(val) => {
                    ...
                    if (commentRequiredStatuses.includes(statusVal)) {
                      const statusLabel =
                        StatusChoices.find((s) => s.value === statusVal)?.label || statusVal;
          ...
          return (
            <div className="text-center">
              <span
                className={cn(
                  "inline-block px-2 py-1 border rounded text-[10px] uppercase font-semibold",
                  getStatusBadgeClass(currentStatus || "")
                )}
              >
                {StatusChoices.find((s) => s.value === currentStatus)?.label || currentStatus}
            ...
            <div className="grid grid-cols-4 gap-0.5 items-center justify-center">
              ...
              <AlertDialog>
                ...
                <AlertDialogContent onClick={(e) => e.stopPropagation()}>
                  ...
                  <AlertDialogFooter>
                    <AlertDialogCancel disabled={deletingTrackingNumber === row.original.tracking_number}>
                      Cancel
                    </AlertDialogCancel>
                    <AlertDialogAction
    ...
    [router, user?.user_id, user?.role, riders, assignRiderMutation, updateOrderMutation, deletingTrackingNumber]
  );
  ...
  return (
    <div className="flex flex-col gap-6 w-full max-w-7xl mx-auto">
      {/* Optional Rider Delivery & Commission Statistics */}
      {showStats && (
      ...
      <div className="mt-4">
        <DataTable
          ...
          onRowClick={(order) => router.push(`/track-order/${order.tracking_number}`)}
          pageCount={pageCount}
      ...
      <Dialog open={viewTrackingNumber !== null} onOpenChange={(open) => !open && setViewTrackingNumber(null)}>
        <DialogContent className="min-w-5xl max-h-[90vh] overflow-y-auto">
          ...
          {viewTrackingNumber && <OrderDetailsView trackingNumber={viewTrackingNumber} />}
        </DialogContent>
      </Dialog>
            ...
            <div className="text-xs text-gray-500 flex flex-col gap-1.5 bg-slate-50 p-3 rounded-sm border border-slate-100">
              ...
              <div className="flex items-center gap-2">
                ...
                <span
                  className={cn(
                    "inline-block px-2 py-0.5 rounded text-[10px] font-medium border",
                    getStatusBadgeClass(statusChangeDialog?.val || "")
                  )}
                >

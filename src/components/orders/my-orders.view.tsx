      ...
      >
        ...
        <div className="flex relative flex-col gap-1.5">
          ...
          <Select
            ...
            onValueChange={(value) => setDraftDeliveryLocation(value || "")}
          >
            <SelectTrigger
              className={cn(
                "w-full !h-10 rounded-xs px-3 text-xs text-gray-500 bg-white shadow-none focus:ring-0 transition-colors",
            ...
            <SelectContent>
              {DeliveryLocationChoices.map((deliveryChoice) => (
                <SelectItem
                  key={deliveryChoice.value}
                  value={deliveryChoice.value}
                >
        ...
        <div className="flex relative flex-col gap-1.5">
          ...
          <Select
            value={draftIsAssigned}
            onValueChange={(value) => setDraftIsAssigned(value || "")}
          >
            <SelectTrigger
              className={cn(
                "w-full !h-10 rounded-xs px-3 text-xs text-gray-500 bg-white shadow-none focus:ring-0 transition-colors",
                appliedIsAssigned ? "border-orange-400" : "border-gray-200",
              )}
            >
            ...
            <SelectContent>
              {AssignedChoices.map((assignedChoice) => (
                <SelectItem
                  key={assignedChoice.value}
                  value={assignedChoice.value}
                >
          ...
          <div className="relative flex flex-col gap-1.5">
            ...
            >
              <SelectTrigger
                className={cn(
                  "w-full !h-10 rounded-xs px-3 text-xs text-gray-500 bg-white shadow-none focus:ring-0 transition-colors",
                  appliedVendorId ? "border-orange-400" : "border-gray-200",
                )}
              >
        ...
        cell: ({ row }) => {
            ...
            return (
              <div className="text-center min-w-[140px]">
                <Select
                  ...
                  onValueChange={(val) => {
                    ...
                    if (commentRequiredStatuses.includes(statusVal)) {
                      const statusLabel =
                        StatusChoices.find((s) => s.value === statusVal)
                          ?.label || statusVal;
                ...
                >
                  ...
                  >
                    <SelectValue placeholder="Select Status">
                      {StatusChoices.find((s) => s.value === currentStatus)
                        ?.label ||
                        currentStatus ||
                        ""}
                    </SelectValue>
          ...
          return (
            <div className="text-center">
              ...
              >
                {StatusChoices.find((s) => s.value === currentStatus)?.label ||
                  currentStatus}
              </span>
            </div>
          );
        ...
        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          onClick={() => {
                            ...
                            const order_ids = selected_ids.filter((id) => {
                              const ord = allOrders.find(
                                (o) => o.tracking_number === id,
                              );
                              return ord?.status !== "ORDER_PLACED";
                            });
                            const hasOrderPlaced =
                              selected_ids.length !== order_ids.length;

                            if (order_ids.length === 0) {
                              toast.error(
                                "Cannot assign rider to orders with status 'Order Placed'",
                              );
                              return;
                            }
                            ...
                            if (hasOrderPlaced) {
                              toast.info(
                                "Skipping orders with status 'Order Placed'",
                              );
                            }
                            ...
                            assignRiderMutation.mutate(
                              { order_ids, rider_id: String(rider.id) },
                              {
                                onSuccess: () => {
                                  toast.success(
                                    `Rider assigned to ${order_ids.length} order(s)`,
                                  );
                                  setSelectedOrderIds({});
                                },
                                onError: (err: any) => {
                                  toast.error(
                                    err?.message ?? "Failed to assign rider",
                                  );
                                },
                              },
                            );
          ...
          >
            ...
            {selectedCount === 0 && (
              <Tooltip>
                <TooltipTrigger
                  render={<HelpCircle className="h-3.5 w-3.5" />}
                />
                <TooltipContent>
        ...
        <DialogContent className="sm:max-w-md">
            ...
            <div className="text-xs text-gray-500 flex flex-col gap-1.5 bg-slate-50 p-3 rounded-sm border border-slate-100">
              <div>
                ...
                <span className="font-semibold text-gray-700">
                  {statusChangeDialog?.trackingNumber}
                </span>
              </div>
              <div className="flex items-center gap-2">
          ...
          <DialogFooter className="gap-2 sm:gap-0">
            ...
            <Button
              size="sm"
              disabled={
                !statusChangeComment.trim() || updateOrderMutation.isPending
              }
              onClick={() => {

...
import { useDeliveryBillDetail } from "./delivery-bills.queries";
...
interface DeliveryBillDetailsViewProps {
  billId: string | number;
  vendorId?: string | number;
}

export function DeliveryBillDetailsView({
  billId,
  vendorId,
}: DeliveryBillDetailsViewProps) {
  ...
  const {
    ...
    isError,
    error,
  } = useDeliveryBillDetail(billId);

  const orders = detail?.orders_detail ?? [];

  const handleBack = () => {
    if (vendorId) {
      router.push(`/dashboard/vendors/${vendorId}/delivery-bills?tab=delivery_bills`);
    } else {
      router.push(`/dashboard/delivery-bills?tab=delivery_bills`);
    }
  };
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute(
      "download",
      `delivery_bill_${detail.bill_number || `bill_${billId}`}.csv`,
    );
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center p-20 gap-3">
        ...
        <p className="text-sm text-gray-500">Loading delivery bill details...</p>
      </div>
    );
  }
    ...
    return (
      <div className="flex flex-col items-center justify-center p-20 gap-4 text-center">
        <p className="text-red-500 font-semibold">
          Error loading delivery bill details
        </p>
        <p className="text-xs text-gray-500 max-w-sm">
          {error instanceof Error
            ? error.message
            : "The requested delivery bill could not be fetched."}
        </p>
          ...
          <div className="flex flex-col gap-6 md:text-right">
            <div>
              ...
              <p className="text-xs text-gray-400 mt-1">
                Bill Date: {formatDate(detail.created_at)}
              </p>
            </div>
